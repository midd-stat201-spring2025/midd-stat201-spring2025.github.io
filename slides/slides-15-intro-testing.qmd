...
p_val <- sum(p_null_vec <= p_hat)/B
```

-   Out of `r B` replications, we saw `r sum(p_null_vec <= p_hat)` instances of $\hat{p} \leq \hat{p}_{obs}$
-   p-value is $\frac{ `r sum(p_null_vec <= p_hat)`}{`r B`} \approx$ `r round(p_val, 2)`
