...
$$\bar{X} \overset{\cdot}{\sim} N\left(\mu, \frac{\sigma}{n} \right)$$ where $\mu$ and $\sigma$ are the population mean and standard deviation, and $\bar{X}$ is the sample mean obtained from a sample of size $n$.
...
3.  **Critical value**: to obtain the middle $\gamma \times 100\%$ part, we use the $\frac{1-\gamma}{2}$ and $\frac{1+\gamma}{2}$ percentiles of the $N(0,1)$ distribution

    -   $z_{(1-\gamma)/2}^{*}$ (lower bound) and $z_{(1+\gamma)/2}^{*}$ (upper bound)

    -   Note: $z_{(1+\gamma)/2}^{*} = - z_{(1-\gamma)/2}^{*}$

    -   Note: the critical value does not depend on the data!
...
$$where the critical value is obtained from $N(0,1)$ distribution. This is obtained directly from the sampling distribution of $\hat{p}$.
:::
