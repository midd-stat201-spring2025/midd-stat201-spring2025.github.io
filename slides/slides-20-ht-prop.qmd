...
If assumptions are met and $H_{0}$ is true, then CLT tells us our **null distribution** is:

$$
\hat{p} \overset{\cdot}{\sim} N\left(p_{0}, \sqrt{\frac{p_{0} (1-p_{0})}{n}} \right)
$$

-   That is: this is how the sample proportion would behave assuming $H_{0}$ is true!
...
-   We calculate a **test statistic**: a quantity that assesses how consistent our sampled data are with $H_{0}$
    ...
    -   For this specific test, our test statistic is the $z$-score from the previous slide, and our *observed* value of the test statistic is:

    ::: fragment
    $$z =\frac{\hat{p}_{\text{obs}} - p_{0}}{\sqrt{\frac{p_{0} (1-p_{0})}{n}}}$$
    :::

    -   Will find this value to the $N(0,1)$ distribution
...
iv. ::: discuss
    Calculate p-value (i.e. set-up the calculation and/or write code)
    :::
...
::: {.column width="60%"}
3.  By CLT, our null distribution is $\hat{p} \overset{\cdot}{\sim} N\left(0.14, \sqrt{\frac{0.14(1-0.14)}{100}} \right)$
...
::: {.column width="60%"}
3.  Since $H_{A}$ is two-sided, we want $$\begin{align*}
